...
export const QuestionModal = ({ selectedQuestion, onClose }: Props) => {
	return (
		<motion.div
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			...
			>
				...
				>
					...
					<motion.span
						...
						className={`inline-flex ml-3 items-center px-3 py-1.5 rounded-full text-sm font-medium ${getRandomBadgeColor()}`}
					>
				...
				>
					...
					<p className="text-gray-600 font-medium">
						Recibida {getTimeAgo(selectedQuestion.createdAt)}
					</p>
				</motion.div>
				<motion.div
					initial={{ opacity: 0, y: 20 }}
